...
## 3.S 화면 카탈로그 (src/screens/ — 디자인 입힘)

### Login (src/screens/Login/)
- 의도: Travelnote 첫 진입 — A안(탑승권 스텁 로그인). 폼 컨테이너가 보딩패스 카드 자체
- 시그니처 번역: 절취선 2줄(헤더↔폼, 폼↔세션) · PASSENGER NAME 라벨 · D-day 대신 SESSION 스텁(자동로그인)
- 액센트 1곳: CTA "계속 이어가기" — 면이 아닌 primary 헤어라인 보더 + 좌측 얇은 코랄 스트라이프
- 토큰: bg-muted/surface · text/text-muted/text-subtle · primary · border-decorative/interactive · radius-lg/full/md · shadow-md(brand hue 틴트) · font-size-xs/sm/base/lg/2xl
- 대비: primary on surface 4.74:1(UI) · text-muted on surface 7.77:1 · text on surface 18.1:1 — 전수 PASS
- 관습 파괴: 폼이 곧 보딩패스. 입력은 박스 대신 카드 위 언더라인
- Stories: Default · WithError · RememberOn
- a11y: prefers-reduced-motion 분기 · 모든 인터랙티브 탭영역 ≥ 44 · aria-describedby로 에러 연결
- 생성일: 2026-06-13
...
- 2026-06-13 — `/design Login` (ds-screen-designer): `src/screens/Login/` 3개 파일 생성. A안 보딩패스 카드 — 절취선 2줄, PASSENGER NAME 라벨, SESSION 스텁(자동로그인 토글), CTA primary 헤어라인 보더 + 좌측 코랄 스트라이프. typecheck PASS · eval 100/100 · 색 페어 전수 PASS.
